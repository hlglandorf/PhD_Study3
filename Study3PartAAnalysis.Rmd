...
bio_dat$HRV <- as.numeric(bio_dat$HRV)
...
# version to use for burnout
cortisol_plot1 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
  mutate(cortisol_conc = cortisol_conc*10) %>%
  select(id, timepoint, cortisol_conc, EXHr, DEVr, RSAr, BURNr) %>%
  rename(Cort = cortisol_conc, EPE = EXHr, DEV = DEVr, RSA = RSAr, BO = BURNr) %>%
  pivot_longer(cols = c(Cort, EPE, DEV, RSA, BO), names_to = "Variable", values_to = "Level")
cortisol_plot1$Variable <- factor(cortisol_plot1$Variable, levels = c("BO", "EPE", "DEV", "RSA", "Cort"))
#for stress
cortisol_plot2 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
  mutate(cortisol_conc = cortisol_conc*50) %>%
  select(id, timepoint, cortisol_conc, STRESS) %>%
  rename(Cort = cortisol_conc, Stress = STRESS) %>%
  pivot_longer(cols = c(Cort, Stress), names_to = "Variable", values_to = "Level")
cortisol_plot2$Variable <- factor(cortisol_plot2$Variable, levels = c("Stress", "Cort"))
...
ggplot(cortisol_plot1%>%filter(Variable %in% c("DEV", "Cort")), aes(timepoint, Level))+
  facet_grid(~id)+
...
ggplot(cortisol_plot2%>%filter(Variable %in% c("Stress", "Cort")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  #geom_line(aes(y = TOTAL), color = "blue")+
  scale_y_continuous(name = "Stress", limits = c(0,25), sec.axis = sec_axis(trans=~./50, name = "Cortisol (ug/dL)"))+
  scale_x_continuous(name = "Timepoint", limits = c(0,12.5))+
  theme_bw()+
    theme(
    axis.title.y = element_text(color="grey1"),
...
ggplot(testosterone_plot1%>%filter(Variable %in% c("DEV", "Testo")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Devaluation", limits = c(0,5), sec.axis = sec_axis(trans=~.*70, name = "Testosterone (pg/mL)"))+
  scale_x_continuous(name = "Timepoint")+
...
tc_plot1 <- dat1 %>% 
  ...
  select(id, timepoint, testosterone_cortisol, EXHr, DEVr, RSAr, BURNr) %>%
  rename(TC = testosterone_cortisol, EPE = EXHr, DEV = DEVr, RSA = RSAr, BO = BURNr) %>%
  pivot_longer(cols = c(TC, EPE, DEV, RSA, BO), names_to = "Variable", values_to = "Level")
tc_plot1$Variable <- factor(tc_plot1$Variable, levels = c("BO", "EPE", "DEV", "RSA", "TC"))
tc_plot1 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
  mutate(testosterone_cortisol = testosterone_cortisol/500) 
tc_plot2 <- dat1 %>% 
...
ggplot(tc_plot1%>%filter(Variable %in% c("DEV", "TC")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Devaluation", limits = c(0,5), sec.axis = sec_axis(trans=~.*500, name = "Testosterone/Cortisol Ratio"))+
  scale_x_continuous(name = "Timepoint")+
...
crp_plot1 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
  mutate(CRP_conc = CRP_conc/200) %>%
  select(id, timepoint, CRP_conc, EXHr, DEVr, RSAr, BURNr) %>%
  rename(CRP = CRP_conc, EPE = EXHr, DEV = DEVr, RSA = RSAr, BO = BURNr) %>%
  pivot_longer(cols = c(CRP, EPE, DEV, RSA, BO), names_to = "Variable", values_to = "Level")
crp_plot1$Variable <- factor(crp_plot1$Variable, levels = c("BO", "EPE", "DEV", "RSA", "CRP"))
crp_plot2 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
...
ggplot(crp_plot1%>%filter(Variable %in% c("DEV", "CRP")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Devaluation", limits = c(0,5), sec.axis = sec_axis(trans=~.*200, name = "C-reactive Protein (pg/mL)"))+
  scale_x_continuous(name = "Timepoint")+
...
ggplot(iga_plot1%>%filter(Variable %in% c("BO", "sIgA")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Total Burnout", limits = c(0,5), sec.axis = sec_axis(trans=~.*70, name = "sIgA Flow Rate (pg/min)"))+
  scale_x_continuous(name = "Timepoint", limits = c(0,12.5))+
  theme_bw()+
    theme(
    axis.title.y = element_text(color="grey1"),
...
ggplot(iga_plot1%>%filter(Variable %in% c("DEV", "sIgA")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Devaluation", limits = c(0,5), sec.axis = sec_axis(trans=~.*70, name = "sIgA Flow Rate (pg/min)"))+
  scale_x_continuous(name = "Timepoint", limits = c(0,12.5))+
  theme_bw()+
    theme(
    axis.title.y = element_text(color="grey1"),
...
ggplot(iga_plot1%>%filter(Variable %in% c("RSA", "sIgA")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Reduced Sense of Accomplishment", limits = c(0,5), sec.axis = sec_axis(trans=~.*70, name = "sIgA Flow Rate (pg/min)"))+
  scale_x_continuous(name = "Timepoint", limits = c(0,12.5))+
  theme_bw()+
    theme(
    axis.title.y = element_text(color="grey1"),
...
bh_plot2 <- dat1 %>% 
  mutate(id = recode(id, "SA_P1" = "Participant A1", "SA_P4" = "Participant A2")) %>%
  mutate(DBP = DBP/5)%>%
  select(id, timepoint, DBP, EXHr, DEVr, RSAr, BURNr, STRESS) %>%
  rename(EPE = EXHr, DEV = DEVr, RSA = RSAr, BO = BURNr, Stress = STRESS) %>%
  pivot_longer(cols = c(DBP, EPE, DEV, RSA, BO, Stress), names_to = "Variable", values_to = "Level")
bh_plot2$Variable <- factor(bh_plot2$Variable, levels = c("BO", "EPE", "DEV", "RSA", "Stress", "DBP"))
...
ggplot(bh_plot2%>%filter(Variable %in% c("Stress", "DBP")), aes(timepoint, Level))+
  facet_grid(~id)+
  geom_line(aes(linetype = Variable))+
  scale_y_continuous(name = "Stress", limits = c(0,25), sec.axis = sec_axis(trans=~.*10, name = "DBP (mmHg)"))+
  scale_x_continuous(name = "Timepoint", limits = c(0,12.5))+
  theme_bw()+
    theme(
    axis.title.y = element_text(color="grey1"),
...
##cortisol
d1_reg_total_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(TOTAL, 0:1)) + d(L(cortisol_conc)), data=zoo_p1)
d1_reg_exh_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(EXH, 0:1)) + d(L(cortisol_conc)), data=zoo_p1)
d1_reg_dev_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(DEV, 0:1)) + d(L(cortisol_conc)), data=zoo_p1)
d1_reg_rsa_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(RSA, 0:1)) + d(L(cortisol_conc)), data=zoo_p1)
d1_reg_stress_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(STRESS, 0:1)) + d(L(cortisol_conc)), data=zoo_p1)
...
##testosterone
d1_reg_total_test <- dynlm(formula = d(testosterone_conc) ~ d(L(TOTAL, 0:1)) + d(L(testosterone_conc)), data=zoo_p1)
d1_reg_exh_test <- dynlm(formula = d(testosterone_conc) ~ d(L(EXH, 0:1)) + d(L(testosterone_conc)), data=zoo_p1)
d1_reg_dev_test <- dynlm(formula = d(testosterone_conc) ~ d(L(DEV, 0:1)) + d(L(testosterone_conc)), data=zoo_p1)
d1_reg_rsa_test <- dynlm(formula = d(testosterone_conc) ~ d(L(RSA, 0:1)) + d(L(testosterone_conc)), data=zoo_p1)
d1_reg_stress_test <- dynlm(formula = d(testosterone_conc) ~ d(L(STRESS, 0:1)) + d(L(testosterone_conc)), data=zoo_p1)
...
d1_reg_total_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(TOTAL, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p1)
d1_reg_exh_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(EXH, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p1)
d1_reg_dev_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(DEV, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p1)
d1_reg_rsa_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(RSA, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p1)
d1_reg_stress_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(STRESS, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p1)
...
##CRP
d1_reg_total_crp <- dynlm(formula = d(CRP_conc) ~ d(L(TOTAL, 0:1)) + d(L(CRP_conc)), data=zoo_p1)
d1_reg_exh_crp <- dynlm(formula = d(CRP_conc) ~ d(L(EXH, 0:1)) + d(L(CRP_conc)), data=zoo_p1)
d1_reg_dev_crp <- dynlm(formula = d(CRP_conc) ~ d(L(DEV, 0:1)) + d(L(CRP_conc)), data=zoo_p1)
d1_reg_rsa_crp <- dynlm(formula = d(CRP_conc) ~ d(L(RSA, 0:1)) + d(L(CRP_conc)), data=zoo_p1)
d1_reg_stress_crp <- dynlm(formula = d(CRP_conc) ~ d(L(STRESS, 0:1)) + d(L(CRP_conc)), data=zoo_p1)
...
##DHEAS
d1_reg_total_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(TOTAL, 0:1)) + d(L(DHEASxflow)), data=zoo_p1)
d1_reg_exh_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(EXH, 0:1)) + d(L(DHEASxflow)), data=zoo_p1)
d1_reg_dev_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(DEV, 0)) + d(L(DHEASxflow)), data=zoo_p1)
d1_reg_dev_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(DEV, 0:1)) + d(L(DHEASxflow)), data=zoo_p1)
d1_reg_rsa_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(RSA, 0:1)) + d(L(DHEASxflow)), data=zoo_p1)
d1_reg_stress_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(STRESS, 0:1)) + d(L(DHEASxflow)), data=zoo_p1)
...
##IgA
d1_reg_total_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(TOTAL, 0:1)) + d(L(IgAxflow)), data=zoo_p1)
d1_reg_exh_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(EXH, 0:1)) + d(L(IgAxflow)), data=zoo_p1)
d1_reg_dev_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(DEV, 0:1)) + d(L(IgAxflow)), data=zoo_p1)
d1_reg_rsa_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(RSA, 0:1)) + d(L(IgAxflow)), data=zoo_p1)
d1_reg_stress_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(STRESS, 0:1)) + d(L(IgAxflow)), data=zoo_p1)
...
##sbp
d1_reg_total_sbp <- dynlm(formula = d(SBP) ~ d(L(TOTAL, 0:1)) + d(L(SBP)), data=zoo_p1)
d1_reg_exh_sbp <- dynlm(formula = d(SBP) ~ d(L(EXH, 0:1)) + d(L(SBP)), data=zoo_p1)
d1_reg_dev_sbp <- dynlm(formula = d(SBP) ~ d(L(DEV, 0:1)) + d(L(SBP)), data=zoo_p1)
d1_reg_rsa_sbp <- dynlm(formula = d(SBP) ~ d(L(RSA, 0:1)) + d(L(SBP)), data=zoo_p1)
d1_reg_stress_sbp <- dynlm(formula = d(SBP) ~ d(L(STRESS, 0:1)) + d(L(SBP)), data=zoo_p1)
...
##dbp
d1_reg_total_dbp <- dynlm(formula = d(DBP) ~ d(L(TOTAL, 0:1)) + d(L(DBP)), data=zoo_p1)
d1_reg_exh_dbp <- dynlm(formula = d(DBP) ~ d(L(EXH, 0:1)) + d(L(DBP)), data=zoo_p1)
d1_reg_dev_dbp <- dynlm(formula = d(DBP) ~ d(L(DEV, 0:1)) + d(L(DBP)), data=zoo_p1)
d1_reg_rsa_dbp <- dynlm(formula = d(DBP) ~ d(L(RSA, 0:1)) + d(L(DBP)), data=zoo_p1)
d1_reg_stress_dbp <- dynlm(formula = d(DBP) ~ d(L(STRESS, 0:1)) + scale(d(L(DBP))), data=zoo_p1)
...
##HRV
d1_reg_total_hrv <- dynlm(formula = d(HRV) ~ d(L(TOTAL, 0:1)) + d(L(HRV)), data=zoo_p1)
d1_reg_exh_hrv <- dynlm(formula = d(HRV) ~ d(L(EXH, 0:1)) + d(L(HRV)), data=zoo_p1)
d1_reg_dev_hrv <- dynlm(formula = d(HRV) ~ d(L(DEV, 0:1)) + d(L(HRV)), data=zoo_p1)
d1_reg_rsa_hrv <- dynlm(formula = d(HRV) ~ d(L(RSA, 0:1)) + d(L(HRV)), data=zoo_p1)
d1_reg_stress_hrv <- dynlm(formula = d(HRV) ~ d(L(STRESS, 0:1)) + d(L(HRV)), data=zoo_p1)
...
##cortisol
d4_reg_total_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(TOTAL, 0:1)) + d(L(cortisol_conc)), data=zoo_p4)
d4_reg_exh_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(EXH, 0:1)) + d(L(cortisol_conc)), data=zoo_p4)
d4_reg_dev_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(DEV, 0:1)) + d(L(cortisol_conc)), data=zoo_p4)
d4_reg_rsa_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(RSA, 0:1)) + d(L(cortisol_conc)), data=zoo_p4)
d4_reg_stress_cor <- dynlm(formula = d(cortisol_conc) ~ d(L(STRESS, 0:1)) + d(L(cortisol_conc)), data=zoo_p4)
...
##testosterone
d4_reg_total_test <- dynlm(formula = d(testosterone_conc) ~ d(L(TOTAL, 0:1)) + d(L(testosterone_conc)), data=zoo_p4)
d4_reg_exh_test <- dynlm(formula = d(testosterone_conc) ~ d(L(EXH, 0:1)) + d(L(testosterone_conc)), data=zoo_p4)
d4_reg_dev_test <- dynlm(formula = d(testosterone_conc) ~ d(L(DEV, 0:1)) + d(L(testosterone_conc)), data=zoo_p4)
d4_reg_rsa_test <- dynlm(formula = d(testosterone_conc) ~ d(L(RSA, 0:1)) + d(L(testosterone_conc)), data=zoo_p4)
d4_reg_stress_test <- dynlm(formula = d(testosterone_conc) ~ d(L(STRESS, 0:1)) + d(L(testosterone_conc)), data=zoo_p4)
...
##testosterone to cortisol
d4_reg_total_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(TOTAL, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p4)
d4_reg_exh_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(EXH, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p4)
d4_reg_dev_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(DEV, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p4)
d4_reg_rsa_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(RSA, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p4)
d4_reg_stress_tc <- dynlm(formula = d(testosterone_cortisol) ~ d(L(STRESS, 0:1)) + d(L(testosterone_cortisol)), data=zoo_p4)
...
##CRP
d4_reg_total_crp <- dynlm(formula = d(CRP_conc) ~ d(L(TOTAL, 0:1)) + d(L(CRP_conc)), data=zoo_p4)
d4_reg_exh_crp <- dynlm(formula = d(CRP_conc) ~ d(L(EXH, 0:1)) + d(L(CRP_conc)), data=zoo_p4)
d4_reg_dev_crp <- dynlm(formula = d(CRP_conc) ~ d(L(DEV, 0:1)) + d(L(CRP_conc)), data=zoo_p4)
d4_reg_rsa_crp <- dynlm(formula = d(CRP_conc) ~ d(L(RSA, 0:1)) + d(L(CRP_conc)), data=zoo_p4)
d4_reg_stress_crp <- dynlm(formula = d(CRP_conc) ~ d(L(STRESS, 0:1)) + d(L(CRP_conc)), data=zoo_p4)
...
##DHEAS
d4_reg_total_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(TOTAL, 0:1)) + d(L(DHEASxflow)), data=zoo_p4)
d4_reg_exh_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(EXH, 0:1)) + d(L(DHEASxflow)), data=zoo_p4)
d4_reg_dev_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(DEV, 0:1)) + d(L(DHEASxflow)), data=zoo_p4)
d4_reg_rsa_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(RSA, 0:1)) + d(L(DHEASxflow)), data=zoo_p4)
d4_reg_stress_dheasf <- dynlm(formula = d(DHEASxflow) ~ d(L(STRESS, 0:1)) + d(L(DHEASxflow)), data=zoo_p4)
...
##IgA
d4_reg_total_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(TOTAL, 0:1)) + d(L(IgAxflow)), data=zoo_p4)
d4_reg_exh_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(EXH, 0:1)) + d(L(IgAxflow)), data=zoo_p4)
d4_reg_dev_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(DEV, 0:1)) + d(L(IgAxflow)), data=zoo_p4)
d4_reg_rsa_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(RSA, 0:1)) + d(L(IgAxflow)), data=zoo_p4)
d4_reg_stress_igaf <- dynlm(formula = d(IgAxflow) ~ d(L(STRESS, 0:1)) + d(L(IgAxflow)), data=zoo_p4)
...
##sbp
d4_reg_total_sbp <- dynlm(formula = d(SBP) ~ d(L(TOTAL, 0:1)) + d(L(SBP)), data=zoo_p4)
d4_reg_exh_sbp <- dynlm(formula = d(SBP) ~ d(L(EXH, 0:1)) + d(L(SBP)), data=zoo_p4)
d4_reg_dev_sbp <- dynlm(formula = d(SBP) ~ d(L(DEV, 0:1)) + d(L(SBP)), data=zoo_p4)
d4_reg_rsa_sbp <- dynlm(formula = d(SBP) ~ d(L(RSA, 0:1)) + d(L(SBP)), data=zoo_p4)
d4_reg_stress_sbp <- dynlm(formula = d(SBP) ~ d(L(STRESS, 0:1)) + d(L(SBP)), data=zoo_p4)
...
##dbp
d4_reg_total_dbp <- dynlm(formula = d(DBP) ~ d(L(TOTAL, 0:1)) + d(L(DBP)), data=zoo_p4)
d4_reg_exh_dbp <- dynlm(formula = d(DBP) ~ d(L(EXH, 0:1)) + d(L(DBP)), data=zoo_p4)
d4_reg_dev_dbp <- dynlm(formula = d(DBP) ~ d(L(DEV, 0:1)) + d(L(DBP)), data=zoo_p4)
d4_reg_rsa_dbp <- dynlm(formula = d(DBP) ~ d(L(RSA, 0:1)) + d(L(DBP)), data=zoo_p4)
d4_reg_stress_dbp <- dynlm(formula = d(DBP) ~ d(L(STRESS, 0:1)) + d(L(DBP)), data=zoo_p4)
...
##HRV
d4_reg_total_hrv <- dynlm(formula = d(HRV) ~ d(L(TOTAL, 0:1)) + d(L(HRV)), data=zoo_p4)
d4_reg_exh_hrv <- dynlm(formula = d(HRV) ~ d(L(EXH, 0:1)) + d(L(HRV)), data=zoo_p4)
d4_reg_dev_hrv <- dynlm(formula = d(HRV) ~ d(L(DEV, 0:1)) + d(L(HRV)), data=zoo_p4)
d4_reg_rsa_hrv <- dynlm(formula = d(HRV) ~ d(L(RSA, 0:1)) + d(L(HRV)), data=zoo_p4)
d4_reg_stress_hrv <- dynlm(formula = d(HRV) ~ d(L(STRESS, 0:1)) + d(L(HRV)), data=zoo_p4)
...
##cortisol
d1r_reg_total_cor <- dynlm(formula = d(TOTAL) ~ d(L(cortisol_conc, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_cor <- dynlm(formula = d(EXH) ~ d(L(cortisol_conc, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_cor <- dynlm(formula = d(DEV) ~ d(L(cortisol_conc, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_cor <- dynlm(formula = d(RSA) ~ d(L(cortisol_conc, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_cor <- dynlm(formula = d(STRESS) ~ d(L(cortisol_conc, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##testosterone
d1r_reg_total_test <- dynlm(formula = d(TOTAL) ~ d(L(testosterone_conc, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_test <- dynlm(formula = d(EXH) ~ d(L(testosterone_conc, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_test <- dynlm(formula = d(DEV) ~ d(L(testosterone_conc, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_test <- dynlm(formula = d(RSA) ~ d(L(testosterone_conc, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_test <- dynlm(formula = d(STRESS) ~ d(L(testosterone_conc, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##testosterone to cortisol
d1r_reg_total_tc <- dynlm(formula = d(TOTAL) ~ d(L(testosterone_cortisol, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_tc <- dynlm(formula = d(EXH) ~ d(L(testosterone_cortisol, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_tc <- dynlm(formula = d(DEV) ~ d(L(testosterone_cortisol, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_tc <- dynlm(formula = d(RSA) ~ d(L(testosterone_cortisol, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_tc <- dynlm(formula = d(STRESS) ~ d(L(testosterone_cortisol, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##CRP
d1r_reg_total_crp <- dynlm(formula = d(TOTAL) ~ d(L(CRP_conc, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_crp <- dynlm(formula = d(EXH) ~ d(L(CRP_conc, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_crp <- dynlm(formula = d(DEV) ~ d(L(CRP_conc, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_crp <- dynlm(formula = d(RSA) ~ d(L(CRP_conc, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_crp <- dynlm(formula = d(STRESS) ~ d(L(CRP_conc, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##DHEAS
d1r_reg_total_dheasf <- dynlm(formula = d(TOTAL) ~ d(L(DHEASxflow, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_dheasf <- dynlm(formula = d(EXH) ~ d(L(DHEASxflow, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_dheasf <- dynlm(formula = d(DEV) ~ d(L(DHEASxflow, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_dheasf <- dynlm(formula = d(RSA) ~ d(L(DHEASxflow, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_dheasf <- dynlm(formula = d(STRESS) ~ d(L(DHEASxflow, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##DHEA-s/Cortisol Ratio
d1r_reg_total_dc <- dynlm(formula = d(TOTAL) ~ d(DHEAS_cortisol, 0:1) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_dc <- dynlm(formula = d(EXH) ~ d(DHEAS_cortisol, 0:1) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_dc <- dynlm(formula = d(DEV) ~ d(DHEAS_cortisol, 0:1) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_dc <- dynlm(formula = d(RSA) ~ d(DHEAS_cortisol, 0:1) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_dc <- dynlm(formula = d(STRESS) ~ d(DHEAS_cortisol, 0:1) + d(L(STRESS)), data=zoo_p1)
...
##IgA
d1r_reg_total_igaf <- dynlm(formula = d(TOTAL) ~ d(L(IgAxflow, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_igaf <- dynlm(formula = d(EXH) ~ d(L(IgAxflow, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_igaf <- dynlm(formula = d(DEV) ~ d(L(IgAxflow, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_igaf <- dynlm(formula = d(RSA) ~ d(L(IgAxflow, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_igaf <- dynlm(formula = d(STRESS) ~ d(L(IgAxflow, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##sbp
d1r_reg_total_sbp <- dynlm(formula = d(TOTAL) ~ d(L(SBP, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_sbp <- dynlm(formula = d(EXH) ~ d(L(SBP, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_sbp <- dynlm(formula = d(DEV) ~ d(L(SBP, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_sbp <- dynlm(formula = d(RSA) ~ d(L(SBP, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_sbp <- dynlm(formula = d(STRESS) ~ d(L(SBP, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##dbp
d1r_reg_total_dbp <- dynlm(formula = d(TOTAL) ~ d(L(DBP, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_dbp <- dynlm(formula = d(EXH) ~ d(L(DBP, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_dbp <- dynlm(formula = d(DEV) ~ d(L(DBP, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_dbp <- dynlm(formula = d(RSA) ~ d(L(DBP, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_dbp <- dynlm(formula = d(STRESS) ~ d(L(DBP, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##HRV
d1r_reg_total_hrv <- dynlm(formula = d(TOTAL) ~ d(L(HRV, 0:1)) + d(L(TOTAL)), data=zoo_p1)
d1r_reg_exh_hrv <- dynlm(formula = d(EXH) ~ d(L(HRV, 0:1)) + d(L(EXH)), data=zoo_p1)
d1r_reg_dev_hrv <- dynlm(formula = d(DEV) ~ d(L(HRV, 0:1)) + d(L(DEV)), data=zoo_p1)
d1r_reg_rsa_hrv <- dynlm(formula = d(RSA) ~ d(L(HRV, 0:1)) + d(L(RSA)), data=zoo_p1)
d1r_reg_stress_hrv <- dynlm(formula = d(STRESS) ~ d(L(HRV, 0:1)) + d(L(STRESS)), data=zoo_p1)
...
##Participant A2
##cortisol
d4r_reg_total_cor <- dynlm(formula = d(TOTAL) ~ d(L(cortisol_conc, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_cor <- dynlm(formula = d(EXH) ~ d(L(cortisol_conc, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_cor <- dynlm(formula = d(DEV) ~ d(L(cortisol_conc, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_cor <- dynlm(formula = d(RSA) ~ d(L(cortisol_conc, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_cor <- dynlm(formula = d(STRESS) ~ d(L(cortisol_conc, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##testosterone
d4r_reg_total_test <- dynlm(formula = d(TOTAL) ~ d(L(testosterone_conc, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_test <- dynlm(formula = d(EXH) ~ d(L(testosterone_conc, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_test <- dynlm(formula = d(DEV) ~ d(L(testosterone_conc, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_test <- dynlm(formula = d(RSA) ~ d(L(testosterone_conc, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_test <- dynlm(formula = d(STRESS) ~ d(L(testosterone_conc, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##testosterone to cortisol
d4r_reg_total_tc <- dynlm(formula = d(TOTAL) ~ d(L(testosterone_cortisol, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_tc <- dynlm(formula = d(EXH) ~ d(L(testosterone_cortisol, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_tc <- dynlm(formula = d(DEV) ~ d(L(testosterone_cortisol, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_tc <- dynlm(formula = d(RSA) ~ d(L(testosterone_cortisol, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_tc <- dynlm(formula = d(STRESS) ~ d(L(testosterone_cortisol, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##CRP
d4r_reg_total_crp <- dynlm(formula = d(TOTAL) ~ d(L(CRP_conc, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_crp <- dynlm(formula = d(EXH) ~ d(L(CRP_conc, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_crp <- dynlm(formula = d(DEV) ~ d(L(CRP_conc, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_crp <- dynlm(formula = d(RSA) ~ d(L(CRP_conc, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_crp <- dynlm(formula = d(STRESS) ~ d(L(CRP_conc, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
d4r_reg_total_dheasf <- dynlm(formula = d(TOTAL) ~ d(L(DHEASxflow, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_dheasf <- dynlm(formula = d(EXH) ~ d(L(DHEASxflow, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_dheasf <- dynlm(formula = d(DEV) ~ d(L(DHEASxflow, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_dheasf <- dynlm(formula = d(RSA) ~ d(L(DHEASxflow, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_dheasf <- dynlm(formula = d(STRESS) ~ d(L(DHEASxflow, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##IgA
d4r_reg_total_igaf <- dynlm(formula = d(TOTAL) ~ d(L(IgAxflow, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_igaf <- dynlm(formula = d(EXH) ~ d(L(IgAxflow, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_igaf <- dynlm(formula = d(DEV) ~ d(L(IgAxflow, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_igaf <- dynlm(formula = d(RSA) ~ d(L(IgAxflow, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_igaf <- dynlm(formula = d(STRESS) ~ d(L(IgAxflow, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##sbp
d4r_reg_total_sbp <- dynlm(formula = d(TOTAL) ~ d(L(SBP, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_sbp <- dynlm(formula = d(EXH) ~ d(L(SBP, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_sbp <- dynlm(formula = d(DEV) ~ d(L(SBP, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_sbp <- dynlm(formula = d(RSA) ~ d(L(SBP, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_sbp <- dynlm(formula = d(STRESS) ~ d(L(SBP, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##dbp
d4r_reg_total_dbp <- dynlm(formula = d(TOTAL) ~ d(L(DBP, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_dbp <- dynlm(formula = d(EXH) ~ d(L(DBP, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_dbp <- dynlm(formula = d(DEV) ~ d(L(DBP, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_dbp <- dynlm(formula = d(RSA) ~ d(L(DBP, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_dbp <- dynlm(formula = d(STRESS) ~ d(L(DBP, 0:1)) + d(L(STRESS)), data=zoo_p4)
...
##HRV
d4r_reg_total_hrv <- dynlm(formula = d(TOTAL) ~ d(L(HRV, 0:1)) + d(L(TOTAL)), data=zoo_p4)
d4r_reg_exh_hrv <- dynlm(formula = d(EXH) ~ d(L(HRV, 0:1)) + d(L(EXH)), data=zoo_p4)
d4r_reg_dev_hrv <- dynlm(formula = d(DEV) ~ d(L(HRV, 0:1)) + d(L(DEV)), data=zoo_p4)
d4r_reg_rsa_hrv <- dynlm(formula = d(RSA) ~ d(L(HRV, 0:1)) + d(L(RSA)), data=zoo_p4)
d4r_reg_stress_hrv <- dynlm(formula = d(STRESS) ~ d(L(HRV, 0:1)) + d(L(STRESS)), data=zoo_p4)
